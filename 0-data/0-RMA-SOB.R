...
j5 <- rma_base %>% 
  ...
  html_nodes(".rteContainer li+ li a") %>% 
  html_attr("href") %>% 
...
# https://www.rma.usda.gov/-/media/RMAweb/SCC-SOB/State-County-Crop-Coverage/
#  sobsccc_1989forward-pdf.ashx?la=en
cov_names <- c("year", "st_fips", "st_abrv", "cty_fips", "cty_name",
               "com_code", "com_name", "ins_plan", "ins_plan_name", "cov_cat",
               "delivery", "cov_lev", "pol_sold", "pol_earn_count",
               "pol_indem_count", "unit_prem_count", "unit_indem_count",
               "net_report_q_type", "net_report_q", "endorse_acres",
               "liability_amt", "prem_amt", "subsidy_amt",
               "private_subsidy_amt", "add_subsidy_amt", "efa_discount_amt",
               "indem_amt", "loss_ratio")
...
sobcov <- bind_rows(j5) %>% 
  mutate_at(vars(-st_abrv, -cty_name, -com_name, -ins_plan_name, -cov_cat,
                 -delivery, -net_report_q_type),
            parse_number) %>% 
  # vars(year, st_fips, cty_fips, com_code, ins_plan, cov_lev,
...
write_csv(sobcov_acres_all, paste0(local_dir, "/sobcov_acres_all.csv"))
write_rds(sobcov_acres_all, paste0(local_dir, "/sobcov_acres_all.rds"))

# FIPS Summaries, let's do total acres
sobcov_acres_com <- sobcov %>%
  mutate(com_name = toupper(com_name)) %>%
  filter(net_report_q_type == "Acres") %>%
  group_by(com_name, fips, year) %>%
  summarise(acres = sumn(net_report_q),
...
write_csv(sobcov_acres_com, paste0(local_dir, "/sobcov_acres_com.csv"))
write_rds(sobcov_acres_com, paste0(local_dir, "/sobcov_acres_com.rds"))
